...
export default function Home(props) {
        ...
        for (let index = 0; index < addresses.length; index++) {
            if (!added.includes(addresses[index])) {
                ...
                if (winner.toLowerCase() === addresses[index]) {
                    ...
                    let extra_ = (parseFloat(ParticipantsToUsd[addresses[index]]) * 359.99999 / parseFloat(totalUsdValue)) / extra
                    console.log("This is the extra_ value: ", extra_, "This is the extra value: ", extra)
            ...
            window.PotContract.methods.potCount().call().then(
                (potCount) => {
                    potEnteredSocket = window.PotContractEvent.events.EnteredPot({ "filter": { potRound: parseInt(_round) }, "fromBlock": start_from_block }, function (error, result) {
                        try {
                            if (!error) {
                                if (!currentRoundHash.includes(((result.returnValues.enteryCount.toLowerCase()))) || true) {
                                    currentRoundHash.push(((result.transactionHash.toLowerCase())))
                                    // _countEntries++
                                    // setTimeout(() => {
                                    setPotActivityInfo(prevState => ({
                                        ...prevState,
    ...
    useEffect(() => {
        ...
        //         targetUrl = 'https://opensea.io/assets/ethereum/0x8b947d5d33c9b664b91d41e8118bdea99979eadb/1'
            ...
            <div className='Home_content1'>
                <div>
                    <UserCPI account={props.PULSEPOT.userInfo.account} potRound={PotActivityInfo.currentPotRoundInfo.round}
                        userTotalusd={(PotActivityInfo.PotDerivedInfo.ParticipantToUsdValue[props.PULSEPOT.userInfo.account.toLowerCase()]) || 0}
                        potTotalUsdValue={PotActivityInfo.PotDerivedInfo.potTotalUsdValue || 0}
                    />
                </div>
                <div>
                    <Wheel setPotActivityInfo={setPotActivityInfo} startTimer={PotActivityInfo.startTimer} listenToCalculatewinner={listenToCalculatewinner} APP_PLSP={props.PULSEPOT[REACT_APP_PLSP]} duration={props.PULSEPOT.potInfo.duration} timeDiff={0} winner={PotActivityInfo.currentPotRoundInfo.winner} potTotalUsdValue={PotActivityInfo.PotDerivedInfo.potTotalUsdValue} isDarkMode={props.isDarkMode} ParticipantToUsdValue={PotActivityInfo.PotDerivedInfo.ParticipantToUsdValue} participants={PotActivityInfo.PotDerivedInfo.participants} />
                </div>
